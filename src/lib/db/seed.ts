...
async function seed() {
  try {
    ...
    const nike = (
      await db.select().from(brands).where(eq(brands.slug, "nike"))
    )[0];
    const shoesCat = (
      await db.select().from(categories).where(eq(categories.slug, "shoes"))
    )[0];
    const runningCat = (
      await db
        .select()
        .from(categories)
        .where(eq(categories.slug, "running-shoes"))
    )[0];
    const lifestyleCat = (
      await db.select().from(categories).where(eq(categories.slug, "lifestyle"))
    )[0];
    const summer = (
      await db
        .select()
        .from(collections)
        .where(eq(collections.slug, "summer-25"))
    )[0];
    const newArrivals = (
      await db
        .select()
        .from(collections)
        .where(eq(collections.slug, "new-arrivals"))
    )[0];
    ...
    for (let i = 0; i < productNames.length; i++) {
        ...
        for (const size of sizeChoices) {
          ...
          const discountedNum =
            Math.random() < 0.3
              ...
              : null;
          const sku = `NIKE-${insertedProduct.id.slice(
            0,
            8
      ...
      const collectionsForProduct: { id: string }[] =
        Math.random() < 0.5
          ? [summer]
          : ([newArrivals, summer].filter(Boolean) as { id: string }[]);

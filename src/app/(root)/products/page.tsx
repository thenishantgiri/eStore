...
function getParamArray(sp: SearchParams, key: string): string[] {
  const v =
    (sp[key] as string | string[] | undefined) ??
    (sp[`${key}[]`] as string | string[] | undefined);
  return toArray(v);
}
...
export default async function ProductsPage({
  ...
  getParamArray(sp, "gender").forEach((g) =>
    activeBadges.push(g[0].toUpperCase() + g.slice(1))
  );
  getParamArray(sp, "size").forEach((s) => activeBadges.push(`Size: ${s}`));
  getParamArray(sp, "color").forEach((c) =>
    activeBadges.push(c[0].toUpperCase() + c.slice(1))
  );
